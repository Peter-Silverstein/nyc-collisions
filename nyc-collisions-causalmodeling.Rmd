...
## Residuals by Time Period and Borough
```{r}
# Dealing with y_rep
...
crashes_withresiduals_grouped <- crashes_withresiduals %>%
  group_by(monthyear, BoroName) %>%
  summarize(mean_meanres = mean(y_residuals_mean), 
            mean_sdres = sd(y_residuals_sd),
            .groups = "drop") %>%
  st_drop_geometry() %>%
  mutate(
...
residuals_ts_boro_plot
```

## COULD I RUN ANOTHER BYM2 MODEL TO DETERMINE HOW MUCH OF THE STDDEV VARIATION IS SPATIAL
