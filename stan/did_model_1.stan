data {
  ...
  matrix[N, K] xs; // design matrix + treatment group + post indicators
  ...
  // DiD Variables
  array[N] int<lower=0, upper=1> did;
}
transformed data {
  vector[N] log_E = log(E);
  vector[N] did_vec = to_vector(did);
...
parameters {
  ...
  sum_to_zero_vector[I] phi_3; // spatial dependency
  real<lower=0> sigma; // variance
  simplex[3] rho; // proportion of variance assigned to each source
  real<lower=1e-6> nb_disp; // negative binomial dispersion parameter
  real theta; // DiD treatment effect
}
transformed parameters {
  // combined temporal effects and random effects
  vector[N] gamma = (sqrt(rho[1]) * phi_1[i_index] + sqrt(rho[2]) * phi_2[j_index] + (sqrt(rho[3] * inv(tau))) * phi_3[i_index]) * sigma;
  
}
model {
  y ~ neg_binomial_2_log(log_E + beta0 + xs_centered * betas + gamma + theta * did_vec, nb_disp);
  beta0 ~ normal(0, 0.5);
  betas ~ std_normal();
  phi_1 ~ std_normal();
  phi_2 ~ std_normal();
...
generated quantities {
  ...
  vector[N] log_lik;
  {
    vector[N] eta = log_E + beta0 + xs_centered * betas + gamma + theta * did_vec;   // centered data
